...
export default function EditMeet() {
  ...
  return (
    <div>
      {meetData && <div>
        <Typography className={classes.title} variant="h4">{meetData.name}</Typography>
        <Typography className={classes.title} variant="h4">Events:</Typography>
        { events &&
        <div>
          <TableContainer className={classes.container} component={Paper}>
          <form onSubmit={ e => handleSubmit(e)}>
      <Table className={classes.table} aria-label="simple table">
        <caption>Note: Athletes who identify as neither men nor women will be allowed to enter whichever event they feel most comfortable in.</caption>
        <TableHead>
          <TableRow>
        ...
        <TableBody>
          {events.map(event => (
            <TableRow hover key={event.id}>
              <TableCell>{event.name}</TableCell>
              <TableCell>{event.distance} {reverseUnits[event.units]}</TableCell>
